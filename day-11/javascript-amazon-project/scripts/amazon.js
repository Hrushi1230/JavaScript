...
document.querySelectorAll('.add-cart').forEach((cart, indx) => {
  cart.addEventListener('click', () => {
    ...
    const selectQuantity = document.querySelector(`.js-quantity-select-${selectProduct.id}`);
    const value = selectQuantity.value * 1;
    Totalcount += value;
    console.log(value);
    const excitingProd = cartProduct.find(p => p.name === selectProduct.name);

    if (excitingProd) {
      excitingProd.count += value;
...
function addedToCart(cart) {
  const addedToCart = cart.parentElement.querySelector('.added-to-cart');
